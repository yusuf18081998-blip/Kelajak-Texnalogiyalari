...
/* ---------- ADMIN PROFIL: ISM/FAMILIYANI SAQLASH (YETISHMAYOTGAN QISM QO'SHILDI) ---------- */
document.getElementById("adminProfileForm").addEventListener("submit", async function (e) {
  e.preventDefault();
  const statusEl = document.getElementById("adminProfUpdateStatus");
  const fullName = document.getElementById("adminProfIsmFamiliya").value.trim();

  if (!fullName) {
    statusEl.textContent = "Ism familiyani kiriting.";
  ...
  const ism = parts[0] || "";
  const familiya = parts.slice(1).join(" ") || "";

  try {
    await updateDoc(doc(db, "users", myUid), { ism: ism, familiya: familiya });
    myData.ism = ism;
    myData.familiya = familiya;
    document.getElementById("adminNameLabel").textContent = ism + " " + familiya;
    statusEl.textContent = "Profil muvaffaqiyatli saqlandi.";
    ...
    statusEl.className = "kt-error-text mt-2";
    showToast("Profilni saqlashda xatolik: " + err.message, "error");
  }
});
