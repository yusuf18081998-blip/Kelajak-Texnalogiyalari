...
import {
  ...
  doc, getDoc, updateDoc, collection, query, where, orderBy, onSnapshot, serverTimestamp, getDocs,
...
onAuthStateChanged(auth, async function (user) {
  ...
  document.getElementById("greetName").textContent = myData.ism;
...
async function loadTodayTopic() {
  ...
  const q = query(collection(db, "ishreja"), where("sinf", "==", myData.sinf), orderBy("sana", "desc"));
...
/* ---------- AI YORDAMCHI (oddiy kalit-so'z asosidagi) ---------- */
const AI_KNOWLEDGE = [
